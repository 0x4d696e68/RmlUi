...
bool StyleSheetContainer::UpdateCompiledStyleSheet(const Context* context)
{
	...
	const Vector2f viewport_scale(context->GetViewportScale());
	...
	for (int media_block_index = 0; media_block_index < (int)media_blocks.size(); media_block_index++)
	{
		...
		for (const auto& property : media_block.properties.GetProperties())
		{
			...
			case MediaQueryId::Width:
				if (vp_dimensions.x != ComputeLength(property.second.GetNumericValue(), font_size, font_size, dp_ratio, vp_dimensions,viewport_scale))
					all_match = false;
				break;
			...
			case MediaQueryId::Height:
				if (vp_dimensions.y != ComputeLength(property.second.GetNumericValue(), font_size, font_size, dp_ratio, vp_dimensions,viewport_scale))
					all_match = false;
				break;
